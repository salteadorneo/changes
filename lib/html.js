...
export function generateHTML(title, results, config = {}, screenshots = []) {
    ...
    const rowsHTML = results.map(result => {
        ...
        const screenshotsRow = result.screenshots && result.screenshots.length > 0
            ? `
      <tr style="border-bottom: 1px solid #eee; background: #fafafa;">
        <td colspan="5" style="padding: 12px 8px;">
          <div style="font-size: 12px; color: #666; margin-bottom: 8px;">📸 Recent captures:</div>
          <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 8px;">
            ${result.screenshots.map(s => `
              <a href="${s.path}" target="_blank" style="text-decoration: none; color: inherit;">
                <img src="${s.path}" style="width: 100%; height: 80px; object-fit: cover; border-radius: 3px; border: 1px solid #ddd; display: block;">
                <div style="font-size: 10px; color: #999; text-align: center; margin-top: 4px; word-break: break-word;">${s.filename.substring(0, 15)}...</div>
              </a>
            `).join('')}
          </div>
        </td>
    ...
    <div class="content">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 12px;">
        ${screenshots.slice(0, 6).map(s => `
          <a href="${s.path}" target="_blank" style="text-decoration: none; color: inherit;">
            <div style="background: #f8f9fa; border: 1px solid #ddd; border-radius: 4px; overflow: hidden; cursor: pointer; transition: all 0.2s; hover:shadow;">
              <img src="${s.path}" style="width: 100%; height: 150px; object-fit: cover; display: block;">
              <div style="padding: 8px; font-size: 11px; color: #666; word-break: break-all; max-height: 32px; overflow: hidden; background: white;">${s.name}</div>
            </div>
          </a>
        `).join('')}
      </div>
