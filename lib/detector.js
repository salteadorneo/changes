...
/**
 * Normalize HTML content for comparison (remove dynamic parts)
 * @param {string} content
 * @returns {string}
 */
function normalizeContent(content) {
    ...
    // Remove timestamps, IDs, and other dynamic attributes
    normalized = normalized.replace(/\d{10,}/g, '[TIMESTAMP]'); // Unix timestamps
    normalized = normalized.replace(/\?[a-zA-Z0-9_-]+/g, ''); // Query parameters
    normalized = normalized.replace(/id="[^"]*"/gi, 'id="[ID]"'); // IDs
    normalized = normalized.replace(/data-[a-z-]*="[^"]*"/gi, ''); // Data attributes
    normalized = normalized.replace(/<!--[\s\S]*?-->/g, ''); // Comments
...
export async function checkContent(source) {
    try {
        ...
        if (source.type === 'html' && source.selector) {
            ...
            // Normalize content for 'content' type to ignore dynamic parts
